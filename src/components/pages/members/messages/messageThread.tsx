...
import { adminUserEle, defVal, fileEle, getCreatedTime, memberBasicinfo, msgEle, msgThread} from "../../../classes/models"
...
export function MemberMessageThread(mainprop:{thread:msgThread,mbi:memberBasicinfo,backy:(action:number)=>void}){
    ...
    function sendMsg() {
        ...
        function finise(fid:string){
            makeRequest.post('sendMsg',{
                body: sc,
                who: mainprop.mbi.getMemberID(),
                tid: mainprop.thread.getThreadId(),
                mail: mainprop.thread.amFrom(mainprop.mbi.getMemberID())?mainprop.thread.getToMail():mainprop.thread.getFromMail(),
                art:fid
            },(task)=>{
                setLoad(false)
                if(task.isSuccessful()){
        ...
        if(art){
            ...
            makeRequest.uploadFile('msg',fileId,mainprop.mbi.getMemberID(),art!, (task)=>{
                if(task.isSuccessful()){
                    finise(fileId)
                }else{
                    setLoad(false)
    ...
    function MessageMini(prop:{ele:msgEle}) {
        return <div id="lshdw" className="vlc" style={{
            ...
            alignSelf: prop.ele.isMe(mainprop.mbi.getMemberID())?'flex-end':'flex-start',
