...
import { msgStat, msgThread, memberBasicinfo, } from "../../../classes/models"
...
export function MemberMessagesList(mainprop:{mbi:memberBasicinfo,actiony:(thread:msgThread, action:number)=>void, backy:()=>void}){
    ...
    function getStats(dontGetMsgs?:boolean){
        setLoad(true)
        setError(false)
        makeRequest.get(`getMyMessagesStat/${mainprop.mbi.getMemberID()}`,{},(task)=>{
    ...
    function getThreads(index:number){
        ...
        setError(false)
        setLoad(true)
        makeRequest.get(`getMyMessages/${mainprop.mbi.getMemberID()}`,{
            start:(index*20),
            count:20
        },(task)=>{
            setLoad(false)
                    ...
                    </div>
                    {
                        threads.slice((showingIndex*20),(showingIndex*20+20)).map((ele,index)=>{
                            return <div id="clk" className="hlc" key={myKey+index+showingIndex*20} onClick={()=>{
                                ...
                                <MyCell text={ele.amFrom(mainprop.mbi.getMemberID())?ele.getToName():ele.getFromName()} />
        ...
        <div className="ctr" style={{
            ...
            <PickMsgReceiver closy={()=>{
                ...
                makeRequest.post('createMsgThread',{
                    from: mainprop.mbi.getFirstName(),
                    from_uid: mainprop.mbi.getMemberID(),
                    to: 'Admin',
                    to_uid: masterID,
                    last_msg:'New Message',
                    subject: subject,
                    from_mail: mainprop.mbi.getEmail(),
                    to_mail:masterEmail
                },(task)=>{
                    setLoad(false)
                    if(task.isSuccessful()){
